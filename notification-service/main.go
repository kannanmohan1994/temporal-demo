...
func initWorkers(temporalClient client.Client) []worker.Worker {
	notifyUserWorker := worker.New(temporalClient, shared.TaskQueueNotifyUserActivity, worker.Options{})
	notifyUserWorker.RegisterActivity(helpers.NotifyUserActivity)

	return []worker.Worker{notifyUserWorker}
}
